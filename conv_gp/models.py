    ...
    def _last_layer(self, H_X, M, filter_size, stride, layer_params=None):
        ...
        if Z is not None:
            ...
            if filter_size != saved_filter_size:
                print(
                    "filter_size {} != {} for last layer. Resetting parameters.".format(filter_size, saved_filter_size))
                Z = None
                q_mu = None
                q_sqrt = None
...
# """
#################
def identity_conv_gen(NHWC_X_GEN, filter_size, feature_maps_in, feature_maps_out, stride):
    conv = IdentityConv2dMean(filter_size, feature_maps_in, feature_maps_out, stride)
    sess = conv.enquire_session()
    random_images, _ = next(NHWC_X_GEN)
    random_images = tf.cast(random_images, tf.float64)
    return sess.run(conv(random_images))


class GeneratorModelBuilder(ModelBuilder):
    def __init__(self, flags, generator, shape, model_path=None):
        self.generator = generator
        self.shape = shape
        super(GeneratorModelBuilder, self).__init__(flags, None, None, model_path)

    def build(self, delete_me=None):
        Ms = parse_ints(self.flags.M)
        feature_maps = parse_ints(self.flags.feature_maps)
        strides = parse_ints(self.flags.strides)
        filter_sizes = parse_ints(self.flags.filter_sizes)
        ...
        self.generator = delete_me
        dataset = tf.data.Dataset.from_generator(self._reshaped_generator, output_types=(tf.float64, tf.float64),
                                                 output_shapes=((27648,), (16, 1)))
        ...
        return DGP_Base(X, y,
                        likelihood=gpflow.likelihoods.MultiClass(10),
                        num_samples=self.flags.num_samples,
                        layers=layers, name='DGP')

    def _conv_layers(self, Ms, feature_maps, strides, filter_sizes, loaded_parameters={}):
        H_X_gen = self.generator
        H_X = None
        layers = []
        for i in range(len(feature_maps)):
            print("\tlayer {} of {}".format(i, len(feature_maps) - 1))
            M = Ms[i]
            feature_map = feature_maps[i]
            filter_size = filter_sizes[i]
            stride = strides[i]
            ...
            conv_layer, H_X = self._conv_layer(H_X_gen, M, feature_map, filter_size, stride, layer_params)
            layers.append(conv_layer)
        return layers, H_X

    def _conv_layer(self, NHWC_X_GEN, M, feature_map, filter_size, stride, layer_params=None):
        if layer_params is None:
            layer_params = {}
        NHWC = self.shape  # NHWC_X.shape
        view = FullView(input_size=NHWC[1:3],
                        filter_size=filter_size,
                        feature_maps=NHWC[3],
                        stride=stride)
        ...
        H_X = identity_conv_gen(NHWC_X_GEN, filter_size, NHWC[3], feature_map, stride)
        if len(layer_params) == 0:
            conv_features = PatchInducingFeatures.from_images(NHWC_X_GEN, M, filter_size, self.shape)
        else:
            conv_features = PatchInducingFeatures(layer_params.get('Z'))
    ...
    def _last_layer(self, H_X, M, filter_size, stride, layer_params=None):
        ...
        if self.flags.last_kernel == 'rbf':
            ...
            view = FullView(input_size=NHWC[1:],
                            ...
                            stride=stride)
            if Z is None:
                from .kernels import _cluster_patches
                Z = _cluster_patches(H_X, M, filter_size)
            ...
            if self.flags.last_kernel == 'conv':
                ...
                raise ValueError("Invalid last layer kernel")
        return SVGP_Layer(kern=kernel, num_outputs=10,
                          mean_function=gpflow.mean_functions.Zero(output_dim=10),
                          white=self.flags.white, Z=Z)

    def _reshaped_generator(self):
        while True:
            X, y = next(self.generator)
            yield X.reshape(-1, np.prod(X.shape[1:]))[0], y.reshape((-1, 1))
